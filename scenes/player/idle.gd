...
func _physics_process(delta: float) -> void:
	if get_parent().current_state != get_parent().idle_state:
		return
	# state_machine에서 현재 상태 값이 idle일 때만 아래 코드를 실행
	...
	# 좌우 입력이 있으면
		get_parent().change_state(get_parent().run_state)
		# run 상태로 변경
	
	if Input.is_action_just_pressed("ui_down") and player.is_on_floor():
	# 땅바닥에 닿아있고, 아래 화살표 키를 누르면
		get_parent().change_state(get_parent().crouch_state)
		# crouch 상태로 변경
